...
if ($op == 'edit') {
    $id                 = $_GET['id'];
    $sql1               = "select * from surat_keluar where id    = '$id'";
    $q1                 = mysqli_query($koneksi, $sql1);
...
if (isset($_POST['simpan'])) { //untuk create
    ...
    if ($tanggal_terima && $nomor && $sifat && $jenis && $isi && $dari && $kepada && $keterangan) {
        if ($op == 'edit') { //untuk update
            $sql1 = "update surat_keluar set tanggal_terima='$tanggal_terima', nomor='$nomor', sifat='$sifat', jenis='$jenis',isi='$isi',dari='$dari', kepada='$kepada', keterangan='$keterangan' where id='$id'";
            $q1   = mysqli_query($koneksi, $sql1);
            if ($q1) {
            ...
            $sql1 = "insert into surat_keluar(tanggal_terima,nomor,sifat,jenis,isi,dari,kepada,keterangan) values ('$tanggal_terima','$nomor', '$sifat','$jenis','$isi','$dari', '$kepada', '$keterangan')";
            $q1   = mysqli_query($koneksi, $sql1);
            if ($q1) {
        ...
        <div class="card">
            ...
            <div class="card-body">
                <table class="table">
                    ...
                    <tbody>
                        <?php
                        $sql2   = "select * from surat_keluar order by id desc";
