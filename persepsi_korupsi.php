...
if ($op == 'delete') {
    $id     = $_GET['id'];
    $sql1   = "delete from persepsi_korupsi where id = '$id'";
...
if ($op == 'edit') {
    $id         = $_GET['id'];
    $sql1       = "select * from persepsi_korupsi where id = '$id'";
    $q1         = mysqli_query($koneksi, $sql1);
...
if (isset($_POST['simpan'])) { //untuk create
    ...
    if ($tahun && $periode && $semester && $nilai && $mutu && $kinerja && $keterangan) {
        if ($op == 'edit') { //untuk update
            $sql1 = "update persepsi_korupsi set tahun='$tahun',periode='$periode',semester='$semester',nilai='$nilai',mutu='$mutu',kinerja='$kinerja',keterangan='$keterangan' where id='$id'";
            $q1   = mysqli_query($koneksi, $sql1);
            if ($q1) {
            ...
            $sql1 = "insert into persepsi_korupsi(tahun,periode,semester,nilai,mutu,kinerja,keterangan) values ('$tahun','$periode','$semester','$nilai','$mutu','$kinerja','$keterangan')";
            $q1   = mysqli_query($koneksi, $sql1);
            if ($q1) {
    ...
    <div class="card">
        ...
        <div class="card">
            <div class="card-header text-white bg-secondary">
                Hasil Survei Persepsi Korupsi BNN Kota Banda Aceh
            </div>
            <div class="card-body">
                <table class="table">
                    <thead>
                    ...
                    <tbody>
                        <?php
                        $sql2   = "select * from persepsi_korupsi order by id desc";
                        ...
                        ?>
                            <tr>
                                ...
                                <td scope="row">
                                    <a href="persepsi_korupsi.php?op=edit&id=<?php echo $id ?>"><button type="button" class="btn btn-warning">Edit</button></a>
                                    <a href="persepsi_korupsi.php?op=delete&id=<?php echo $id ?>" onclick="return confirm('Yakin mau delete data?')"><button type="button" class="btn btn-danger">Delete</button></a>
