...
if ($op == 'delete') {
    $id     = $_GET['id'];
    $sql1   = "delete from tes_urin_swasta where id = '$id'";
...
if ($op == 'edit') {
    $id         = $_GET['id'];
    $sql1       = "select * from tes_urin_swasta where id = '$id'";
    $q1         = mysqli_query($koneksi, $sql1);
...
if (isset($_POST['simpan'])) { //untuk create
    ...
    if ($nama && $kecamatan && $tanggal && $tempat && $keterangan) {
        if ($op == 'edit') { //untuk update
            $sql1 = "update tes_urin_swasta set nama='$nama',kecamatan='$kecamatan',tanggal='$tanggal',tempat='$tempat',keterangan='$keterangan' where id='$id'";
            $q1   = mysqli_query($koneksi, $sql1);
            if ($q1) {
            ...
            $sql1 = "insert into tes_urin_swasta(nama,kecamatan,tanggal,tempat,keterangan) values ('$nama','$kecamatan','$tanggal','$tempat','$keterangan')";
            $q1   = mysqli_query($koneksi, $sql1);
            if ($q1) {
        ...
        <div class="card">
            ...
            <div class="card-body">
                <table class="table">
                    ...
                    <tbody>
                        <?php
                        $sql2   = "select * from tes_urin_swasta order by id desc";
                        ...
                        ?>
                            <tr>
                                ...
                                <td scope="row">
                                    <a href="tes_urin_swasta.php?op=edit&id=<?php echo $id ?>"><button type="button" class="btn btn-warning">Edit</button></a>
                                    <a href="tes_urin_swasta.php?op=delete&id=<?php echo $id ?>" onclick="return confirm('Yakin mau delete data?')"><button type="button" class="btn btn-danger">Delete</button></a>
